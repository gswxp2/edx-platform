    ...
    def test_content_type_gate_for_block(self):
        ...
        self.assertIn(
            ...
            ContentTypeGatingService()._content_type_gate_for_block(
                self.user, blocks_dict['graded_1'], course, course['course'].id
            ).content
        )
        ...
        self.assertEquals(
            None,
            ContentTypeGatingService()._content_type_gate_for_block(
                self.user, blocks_dict['not_graded_1'], course, course['course'].id
            )
        )

    def test_check_children_for_content_type_gating_paywall(self):
        ''' Verify that the method returns a content type gate when appropriate '''
        course = self._create_course()
        blocks_dict = course['blocks']
        CourseEnrollmentFactory.create(
        ...
        self.assertEquals(
            ...
            ContentTypeGatingService().check_children_for_content_type_gating_paywall(
                blocks_dict['vertical'], course, course['course'].id
            )
        )
